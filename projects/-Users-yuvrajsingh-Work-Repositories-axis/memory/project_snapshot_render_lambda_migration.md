...
**All of Jared's three points are closed (2026-09-24).** axis #1507 merged and live in prod (presigned downloads, kill switch lifted); a prod recording generated an article with 5 snapshots, 0 Lambda errors. infrastructure#161 merged and applied: replication to `axis-remotion-replica-uswest2` (private, versioned, same lifecycle), role `remotion-s3-replication-role` in the `remotion/` root. Gotchas learned: S3 replication does not backfill existing objects, and `remotion lambda sites create` skips unchanged files, so to seed the replica I had to `aws s3 rm` the `sites/` prefix and redeploy the three sites. Do NOT backfill with `aws s3 cp --metadata-directive REPLACE`: it resets every content type to `binary/octet-stream`, which would break the Lambda's browser loading the bundle. Site deploys: `AWS_PROFILE=remotion-production REMOTION_SITE_NAME=<site> pnpm run deploy:site` from `packages/snapshot-composition` (script passes `--privacy=no-acl`).

Known UX wrinkle: a freshly generated article can show a snapshot as a dark box for a moment; renders finish before the draft is persisted, so that is the `<img>` box (width/height reserved) waiting on the CDN fetch in dark mode, not a render still running. `snapshot-node.tsx` has no loading placeholder for an image that has a URL.
...
**Observability (2026-09-25):** axis #1522 adds `recorder.snapshot.render` span + `axis.recorder.snapshot.render_ms` / `.render.cost_usd` metrics + Sentry on swallowed draft renders. infrastructure#163 adds `remotion/alarms.tf` (Errors/Throttles/Concurrency>=80/Duration>=720s to SNS `remotion-render-alarms`, email yuvraj@groovehq.com, copied from github-actions-runner-groove). Jared applies infra PRs now too. After apply the SNS email subscription must be confirmed. My SSO plan fails on iam:GetRole for the replication role; pre-existing.
