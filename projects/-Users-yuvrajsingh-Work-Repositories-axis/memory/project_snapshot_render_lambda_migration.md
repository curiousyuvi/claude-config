...
**Terraform rebuild is PR GrooveHQ/infrastructure#160 (opened 2026-09-24, branch `ys/feat/remotion-terraform-root`), waiting on Matt's review and two applies (`src/` both workspaces, then `remotion/`).** Matt approved the separate-root approach in Slack: the main root's provider 4.45.0 cannot be bumped without tearing down the production EKS cluster. New root `remotion/`: bucket `remotionlambda-useast1-axis`, function `remotion-render-4-0-526-mem10240mb-disk10240mb-900sec`, log group, concurrency 100, zip committed under `remotion/lambda/`. Real plan: 8 add / 0 change / 0 destroy. Run Terraform via `docker run --rm --env-file <creds> -v $PWD:/infrastructure groove/infrastructure:1.3.0` (write `aws configure export-credentials --profile groove --format env` to a file; `eval` is blocked in my shell).

**After Matt applies, the axis follow-up:** restore `REMOTION_LAMBDA_FUNCTION_NAME` in all three env files; change `REMOTION_LAMBDA_SERVE_URL` to `https://remotionlambda-useast1-axis.s3.us-east-1.amazonaws.com/sites/axis-snapshots-<env>/index.html`; replace the anonymous `fetch(still.url)` / `progress.outputFile` download in `kb-snapshot-lambda-render.service.ts` with `presignUrl` from `@remotion/lambda-client` (renders/ is private now); redeploy the three sites from `packages/snapshot-composition` with `sites create --privacy no-acl` using the render user's keys (public-read ACLs are blocked). Then test a render and ping Jared to re-check. Replication was deferred; Jared said "enable for now" so it may come back.
